# Investigation Notes – AppInit DLL Persistence

## Investigation Overview

This investigation examines a suspected **AppInit DLL persistence** mechanism on a Windows endpoint using registry artifacts, a controlled benign DLL, process activity, and Sysmon telemetry.

The investigation focuses on determining whether the available evidence supports actual DLL loading rather than treating the presence of a persistence-related registry location as proof of execution.
...
The expected evidence chain was:
...
The captured evidence confirms only parts of this chain.
...
The investigation confirmed the preparation of the lab environment and generated process and file telemetry.

However, the available evidence does not establish the complete AppInit execution chain.

The missing compiled DLL, captured `LoadAppInit_DLLs : 0` state, absence of demonstrated Event ID 7 evidence, and lack of the expected marker file prevent the investigation from conclusively confirming successful AppInit DLL persistence.

> **Final assessment: AppInit DLL persistence was investigated, but successful DLL loading and persistence were not conclusively demonstrated from the captured evidence.**
